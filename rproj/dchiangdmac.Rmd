...
```{r}
library(readr)
instructors <- read_csv("~/Downloads/data/uwm/instructors.csv")
#View(instructors)
summary(instructors)
```
...
```{r}
library(readr)
teachings <- read_csv("~/Downloads/data/uwm/teachings.csv")
#View(teachings)
```
...
```{r}
library(readr)
sections <- read_csv("~/Downloads/data/uwm/sections.csv")
#View(sections)
```
...
```{r}
library(readr)
grades <- read_csv("~/Downloads/data/uwm/grade_distributions.csv")
#View(grades)
```
...
```{r}
library(readr)
course_offerings <- read_csv("~/Downloads/data/uwm/course_offerings.csv")
...
```{r}
write.csv(df5, '~/Downloads/data/gpa_instr_term.csv')
```
...
4. But most challenging task is matching the professor names. 
...
Often the names do not match exactly, such as MATTHEW HERNDON to Matt Herndon.

### Using Prof. Matt Herndon of Physics Dept (rating: 2.0) as example

Many scraper projects for rate-my-professor site exists in GitHub 
and generally requires HTTP request to the RESTful API the rating site 
provides without static page content scraping. 

For example, data per professor can be retrieved as JSON through the REST URL call:

[get](https://www.ratemyprofessors.com/filter/professor/?&page=1&filter=teacherlastname_sort_s+asc&query=MATT+HERNDON&queryoption=TEACHER&queryBy=schoolId&sid=1256)

where _*sid*_ is the School ID for Univ of WI at Madison, and the client 
may need to experiment with possible variation of professor name.
